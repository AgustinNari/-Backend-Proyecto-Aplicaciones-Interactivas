...
public Page<ProductListDTO> findProductsWithAggregates(ProductFilter filter, Pageable pageable) {
    ...
    if (filter != null) {
        ...
        if (filter.getCategoryIds() != null && !filter.getCategoryIds().isEmpty()) {
            ...
            Expression<Long> p2Id = p2.get("id");
            catExist.select(p2Id);
            catExist.where(cb.equal(p2Id, root.get("id")), joinCat.get("id").in(filter.getCategoryIds()));
        ...
        if (filter.getMinAvgRating() != null) {
            ...
            avgExist.having(cb.greaterThanOrEqualTo(cb.avg(rAvg.get("rating")), filter.getMinAvgRating()));
        ...
        if (filter.getMinDiscountPercent() != null) {
            ...
            Predicate condSeller = cb.equal(d.get("targetSeller").get("id"), root.get("seller").get("id"));
            ...
            catMatch.where(cb.equal(p3.get("id"), root.get("id")), cb.equal(cjoin.get("id"), d.get("targetCategory").get("id")));
            Predicate condCategory = cb.exists(catMatch);

            Predicate anyTarget = cb.or(condProduct, condSeller, condCategory);
            discExist.where(anyTarget, condType, condActive, condValue);
    ...
    Map<Long, List<Long>> productCategoriesMap = loadCategoryIdsForProducts(productIds);
    ...
    for (ProductListDTO dto : dtos) {
        ...
        List<Long> catIds = productCategoriesMap.get(pid);
        dto.setCategories(catIds != null ? catIds : Collections.emptyList());
    ...
    private Map<Long, List<Long>> loadCategoryIdsForProducts(List<Long> productIds) {
        if (productIds == null || productIds.isEmpty()) return Collections.emptyMap();
        List<Object[]> rows = em.createQuery(
                "SELECT p.id, c.id FROM Product p JOIN p.categories c WHERE p.id IN :ids", Object[].class)
        ...
        for (Object[] r : rows) {
            Long pid = (Long) r[0];
            Long cid = (Long) r[1];
            tmp.computeIfAbsent(pid, k -> new ArrayList<>()).add(cid);
        }
        return tmp;
    }
    ...
    private Map<Long, DiscountBest> loadBestDiscountsForProducts(List<Long> productIds,
                                                                 Map<Long, List<Long>> productCategoriesMap,
