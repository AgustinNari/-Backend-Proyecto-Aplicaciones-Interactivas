...
public class OrderService {
    ...
    public OrderDto createOrder(OrderCreateDto dto, Integer buyerId) throws BadRequestException, OutOfStockException, ProductNotFoundException {
        ...
        Map<Integer, List<DigitalKey>> assignedKeysByProduct = new HashMap<>();

      
        List<Integer> productIdsToLock = dto.items().stream().map(OrderItemCreateDto::productId).distinct().sorted().collect(Collectors.toList());
        ...
        try {
            ...
            for (OrderItem item : savedOrder.getItems()) {
                ...
                for (int i = 0; i < item.getQuantity(); i++) {
                    ...
                    digitalKeyRepository.creatKey(assignedKey);
    ...
    public OrderDto getOrderById(Integer orderId, Integer requestingUserId) {
    ...
    public List<OrderDto> listOrdersForBuyer(Integer buyerId) {
